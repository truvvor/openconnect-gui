...
// needed by vpnc.
//
// openconnect-gui: patched to honor OCGUI_NO_DEFAULT_ROUTE (split tunnel).
// When that env var is set, the client does NOT install a default route; only
// the VPN's own subnet (on-link) and any server-pushed split-include routes are
// added, so all other traffic keeps using the normal connection.
//
...
// openconnect-gui: split-tunnel toggle. When set, never install a default route.
var OCGUI_NO_DEFAULT_ROUTE = env("OCGUI_NO_DEFAULT_ROUTE") ? true : false;
...
case "connect":
	...
	if (OCGUI_NO_DEFAULT_ROUTE) {
		echo("Split-tunnel: OCGUI_NO_DEFAULT_ROUTE set -> default route will NOT be installed.");
	}
	...
	if (OCGUI_NO_DEFAULT_ROUTE || env("CISCO_SPLIT_INC") || REDIRECT_GATEWAY_METHOD != 0) {
		// No interface gateway -> Windows does NOT add a default route; only the
		// tunnel's own subnet becomes on-link (reachable via the VPN).
	...
	} else if (OCGUI_NO_DEFAULT_ROUTE) {
		echo("Split-tunnel: default route suppressed by client policy; only the VPN subnet is routed via the tunnel.");
	...
	if (env("INTERNAL_IP6_ADDRESS")) {
		...
		if (env("CISCO_IPV6_SPLIT_INC")) {
			...
			echo("Split-tunnel: IPv6 default route suppressed by client policy.");
		}
...
case "disconnect":
	...
	// Restore direct route (only meaningful if we had overridden it)
	if (!OCGUI_NO_DEFAULT_ROUTE) {
